...
class Tachometer():
    ...
    def moveto(self,value,tag):
        ...
        self.canvas.coords(self.needle,self.radius+720,self.radius+30,self.radius+self.radius*m.cos((value*self.slope+self.intercept)*m.pi/180)+720,self.radius-self.radius*m.sin((value*self.slope+self.intercept)*m.pi/180)+30)
    ...
    #needle        
    def __init__(self,canvas,tag,bg="#ffffff",needlecolor="#0d47a1",markscolor="#000000",Range=(0,30),digitscolor="#ff2222"):
        self.range=Range
        ...
        u=0
        #Speed markings
        for i in range(-10,195,20):
            if(i%10==0):
                #Main speed markings
                x1,y1=self.radius+(self.radius-20)*m.cos(i*m.pi/180)+720,self.radius-(self.radius-20)*m.sin(i*m.pi/180)+30
                canvas.create_line(x1,y1,self.radius+self.radius*m.cos(i*m.pi/180)+720,self.radius-self.radius*m.sin(i*m.pi/180)+30,fill=markscolor,width=2) 
                self.range_marks.append(canvas.create_text(x1+u,y1+q,text=int((i+10)*(self.range[1]-self.range[0]-20)/200.6+self.range[0]),font=("Courier",int(self.radius/10)),fill=digitscolor))
            else:
                #In between markings
        ...
        canvas.create_oval(self.radius+710,self.radius+20,self.radius+730,self.radius+40,fill="#ffff00")
